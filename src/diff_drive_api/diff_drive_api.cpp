...
{
  r_ = distance_to_goal;
  delta_ = slam_car::wrapAngle(delta);
  theta_ = slam_car::wrapAngle(theta);
}
...
{
  double angle_diff = slam_car::wrapAngle(theta_ - delta_);
...
// 参见论文: A Smooth Control Law for Graceful Motion of Differential Wheeled Mobile Robots in 2D Environment
void DiffDrivePoseController::controlPose()
{
  double atan2_k1_theta = std::atan2(-k_1_*theta_, 1.0);

  // 曲率?
  // delta_: current heading of the base [rad]
  // theta_: direction of the pose goal [rad]
  cur_ = (-1 / r_)
      * (k_2_ * (delta_ - atan2_k1_theta) +
         (1 + (k_1_ / (1 + std::pow((k_1_ * theta_), 2)))) * sin(delta_));

  v_ = v_max_ / (1 + beta_ * std::pow(std::abs(cur_), lambda_));

  v_ = enforceMinVelocity(v_, v_min_movement_); //最小速度下线
  v_ = enforceMinMax(v_, v_min_, v_max_); //强制最小(最大负速度值，但一般我们不允许后退，故v_min_movement_ = v_min)和最大速度(最大正速度值)
